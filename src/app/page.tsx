...
function loadPageDataForLocale(locale: string | undefined): HomePageLocaleData {
  ...
  const aboutConfig = getPageConfig<{ profile?: { research_interests?: string[]; quote?: string[] }; sections?: SectionConfig[] }>('about', locale);
  const researchInterests = aboutConfig?.profile?.research_interests;
  const profileQuote = aboutConfig?.profile?.quote;
